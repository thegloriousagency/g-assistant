...
export type DropzoneProps = Partial<DropzoneOptions> & {
  src?: File[];
  className?: string;
  onDrop?: (
    acceptedFiles: File[],
...
export const Dropzone = ({
  ...
  return (
    ...
    >
      <Button
        className={cn(
          "relative flex h-auto w-full flex-col overflow-hidden p-8",
...
export const DropzoneEmptyState = ({ children, className }: DropzoneEmptyStateProps) => {
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center", className)}>
      ...
      <p className="my-2 w-full text-pretty text-sm font-medium">
        Upload {maxFiles === 1 ? "a file" : "files"}
      </p>
      <p className="w-full text-pretty text-xs text-muted-foreground">
        Drag and drop or click to upload
      </p>
      {caption && <p className="text-pretty text-xs text-muted-foreground">{caption}.</p>}
    </div>
  );
};
